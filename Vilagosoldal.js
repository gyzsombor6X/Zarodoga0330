...
export default class App extends Component {
  ...
  async getMovies() {
    try {
      const response = await fetch(IP.ipcim+'vilagosoldal');
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View>
                ...
                <Text style={{color: 'cyan', fontSize: 25, textAlign: 'center'}}>{item.name}</Text>
                <Image source={{uri:'https://gyure-zsombor.dszcbaross.tk/'+item.image}} style={{width:300, height:300, alignSelf: "center"}} />
                {/*                                               *** Kedvelés gomb ***                                                                */}
